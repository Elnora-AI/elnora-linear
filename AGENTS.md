...
| Slack bridge — DM assignees with MEDIUM-tier questions | `elnora-linear curator-slack-bridge <tick\|post-pending\|resolve> [--dry-run] [-v]` — needs `pip install slack-sdk` + `SLACK_BOT_TOKEN` + `OPENROUTER_API_KEY`; see `bridges/slack/README.md` |
...
- Long markdown goes through a file: `issues create`/`issues update --description-file <path>`, `comments create`/`comments update --body-file <path>` (`-` reads stdin). Don't inline it with `--description "$(cat ...)"` — the shell mangles fenced blocks, backticks and quotes. Inline and file options are mutually exclusive; an unreadable, empty or non-UTF-8 file exits 2. Write the file as UTF-8 — on Windows PowerShell use `Set-Content -Encoding utf8`, since `>` and `Out-File` default to UTF-16LE and plain `Set-Content` to ANSI.
