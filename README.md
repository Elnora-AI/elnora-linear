...
- **Markdown bodies come from files.** `issues create`/`issues update` take `--description-file`, `comments create`/`comments update` take `--body-file` (`-` reads stdin), so fenced code blocks and quotes never pass through the shell. The inline and file options are mutually exclusive, and an unreadable, empty, or non-UTF-8 file is an error rather than a silent bad write — no encoding guessing, so a file written with the wrong encoding is caught before anything is sent.
