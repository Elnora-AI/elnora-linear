...
- **Markdown bodies come from files.** `issues create`/`issues update` take `--description-file`, `comments create`/`comments update` take `--body-file` (`-` reads stdin), so fenced code blocks and quotes never pass through the shell. The inline and file options are mutually exclusive, and an unreadable or empty file is an error rather than an empty write.
